...
| F2 | **Interactive pose/3D viewer** — ✅ core shipped: `PoseCanvas` (2D skeleton + bar-path trail on the clean video, rAF-synced) via a "Live pose" toggle, **plus `Pose3D`** (three.js 3D skeleton from the metric world landmarks, orbit/zoom, PiP over the video) via a "3D" toggle; `lib/pose/track.ts` parses the persisted track. ⏳ velocity graph synced to playback, per-rep chapters | T5 | Makes the system feel alive; foundation for all compare UI | M |
