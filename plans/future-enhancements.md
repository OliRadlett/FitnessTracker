...
> **Decision 2026-09-20:** open items triaged into `plans/backlog-2026-09-20.md` (B-08/B-09/B-10/B-11/B-12/B-13/B-19/B-20/B-21/B-22/B-25/B-32/B-33/B-34/B-35). Deferred: §3.13 clustering, BUG-045 rotation. Separate case: §0.1 stash investigation. Scope detail stays here.

> **Date**: 2026-09-08 · **Status**: In progress — Phases A/B/C + Phase D (§3.6–3.10) + §3.12, §3.14, §3.15 shipped to `prod` (2026-09-08 release); **Phase E (video + analytics)** in progress — §3.12 Health-alert tuning + new signals done, §3.14 Race-day prep PDF done, §3.15 stale-data refresh done, §3.11 Adaptive training suggestions done, **§3.13 Ride segment analysis done**, **§3.16 3D replay MVP done** (activity fly-through; 3D route view + side-by-side comparison remain), **§1.2 Activities list enrichment done**, **§1.3 Post-sync background activity analysis done** (ride analytics cached in `Activity.context` at sync time). **Phase D remaining §3.7-live-offline done** (explicit Live Lift offline mode). **§0.2 doc reconcile done** (2026-09-08; merged duplicate `routes/` CODEMAP blocks + orphan `activities/` header). **§1.1 Strength video system — done** (backend 2026-09-08, frontend 2026-09-09): `LiftVideo` model (`lift_videos` table, migration 047), `/api/v1/lifting/videos` API (URL-only embeds + R2 presigned upload scaffold, 501 without S3 creds), R2 config in `Settings`. Frontend: `VideoEmbed` component (YouTube/Vimeo iframe + R2 `<video>`), `VideoGalleryModal` (per-session/PR gallery), `LiftVideoForm` (add URL-only or upload with R2 presigned PUT), `/lifting/videos` Video Bank page with source/exercise/date filters, per-session + per-PR video chips on lifting page, sidebar nav item. URL-only mode end-to-end; upload mode degrades gracefully. **R2 uploads fully implemented (2026-09-09)** — object delete on video deletion, CORS bootstrap script, server-side validation, signing fixes; pending the user creating the Cloudflare bucket/token (see `docs/R2_SETUP.md`). **§3.17 changelog done** (2026-09-09). §1.4 and §3.16 remaining sub-items (deferred on DEM tiles) not done.
...
- [x] Cloudflare R2 presigned PUT (≤250MB, mp4/quicktime/webm), presigned GET stream URLs, `LiftVideo` model (migration 047), graceful 501 without S3 creds. — **R2 fully implemented (2026-09-09)**: presigned PUT/GET + object delete on video deletion, CORS bootstrap (`python -m app.scripts.r2_bootstrap`), server-side size/type validation at the upload-url + create endpoints, collision-safe keys, `region_name="auto"` signing fix, key no longer built from a raw `User` object. Backend `boto3>=1.34` dependency added. Awaiting the user to create the Cloudflare bucket/token and set the `R2_*` env vars — walkthrough: `docs/R2_SETUP.md`. **URL/embed mode removed (2026-09-09, migration 048)**: videos are R2-only — `source`/`external_url` columns dropped, `LiftVideoForm` upload-only, `VideoEmbed` R2 `<video>` only.
...
| BUG-048 | **Deploy Caddyfile placeholder ACME email** `admin@example.com` | `.github/workflows/deploy.yml` | XS | ops — ✅ fixed 2026-09-09 (now `${{ secrets.ACME_EMAIL || 'admin@example.com' }}`) |
...
### 3.16 3D cycle activity visualisations (P2, L) ✅ Done (2026-09-09)
...
- [x] **3D route view** — routes page detail: terrain extrusion + route polyline draped on it (replaces/augments the 2D `ElevationProfile`); zoom/rotate/pitch; quality / surface colouring overlaid. — **Done (2026-09-09)**: the DEM blocker was removed by using the free **Open-Meteo Elevation API** (Copernicus GLO-90, 90m, no API key — same provider as the weather caches, so no new account/credentials). `RouteDetailPanel`'s Map & Profile tab gains a **2D / 3D Terrain** toggle; `components/routes/Route3D.tsx` (three.js, `next/dynamic ssr:false`) drapes the polyline over a ≤100-point DEM heightmap (`computeGrid` in `lib/route3d.ts`), vertex-coloured by **elevation or slope (gradient~)**, with start/end markers, orbit/rotate/pitch via `OrbitControls`, WebGL fallback, and a graceful flat-drape (route's own `elevation_profile`, or bare line) when the DEM fetch fails. Copernicus attribution shown. 19 unit tests in `src/__tests__/route3d.test.ts`; `/routes` first-load stays ~244 kB (three in an on-demand chunk). Per-point surface/quality colouring is still aggregate-only — colour modes use elevation/gradient (real per-point data).
- [x] **Tech stack** — evaluate `maplibre-gl` (terrain 3D, open-source successor to the bundled Leaflet, lightest option with raster/elevation tiles) vs `deck.gl` (large stream-point rendering, best for overlays) vs `three.js` (full control, heaviest). Likely: **MapLibre GL terrain for routes + a lightweight three.js (or MapLibre camera path) fly-through for replay**. — **Decision**: `three` ^0.185 for the replay MVP (no tiles/API keys needed — local-plane projection), as streamed-path overlays are the primary use. MapLibre **not needed** — the route view reuses the same three.js path projection and gets its DEM from the keyless Open-Meteo elevation endpoint.
...
- [x] **Caddyfile ACME email** — real contact in `deploy.yml` heredoc (BUG-048). — **Done (2026-09-09)**: heredoc now uses `${{ secrets.ACME_EMAIL || 'admin@example.com' }}` — set the `ACME_EMAIL` repo secret to the real contact.
...
- [x] **Component tests** — only 5 Vitest files (Chart, lifting-reference, fetch, ErrorBoundary). Extend: `useLiveSession` queue logic, sidebar nav, `formatDistance` unit/locale hook, goal/projection components. — **Done (2026-09-09)**: added 4 files — `video-embed` (embed URL conversion for all YouTube/Vimeo variants, host detection, VideoChip count/plural), `lift-video-form` (render/close, mode switch, required-URL native validation, session/PR dropdowns), `mobile-route-detail-sheet` (null render, backdrop + panel close), `changelog` (expand/collapse, dates). Suite now 100 tests across 10 files.
