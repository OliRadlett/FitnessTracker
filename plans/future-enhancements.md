...
> **Date**: 2026-09-07 · **Status**: In progress — Phase A §2.1–2.3 done & pushed; Phase B (Part 4.1/4.3/4.4, §3.12, §3.1 backend, §5.1–5.3) done, merged to `prod`, deployed; Phase C §3.5 + §3.1 UI done (unshipped), §3.3 result-logging + race-day notify done (unshipped), §3.2/3.6/3.8 pending; unstarted §0.2, §1.1–1.4, Part 3 (D onwards).
...
3. **Phase C — user-facing (Part 3.1–3.5)**: weight UI, Health page, race results, ⌘K search, notifications page. ✅ §3.1 UI + §3.5 + §3.3 (result logging, race-day notify) done on `main` (unshipped — awaiting §3.2 + §3.4 before release).
