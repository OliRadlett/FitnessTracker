...
Backend registry [`CHART_REGISTRY`](../backend/app/api/charts.py) → [`ChartService`](../backend/app/services/charts.py) → frontend [`Chart`](../frontend/src/components/charts/Chart.tsx) renders Recharts (line, bar, scatter, area, pie + custom CSS-grid heatmap). 28 charts registered: weekly_tss, estimated_1rm_history, weekly_volume, sleep_quality_trend, whoop_strain_trend, training_load, ftp_history, stream_power_curve, power_zones, hr_zone_distribution, daily_tss, exercise_progress, power_curve_comparison, strain_vs_recovery, recovery_vs_performance, hrv_trend_detailed, weight_trend, training_load_balance, rest_day_analysis, vo2max_trend, decoupling_trend, periodization, ramp_rate, wkg_power_curve, power_duration_percentile, consistency_heatmap, sleep_consistency, strength_balance. Reference areas supported for zone coloring (per-axis via `y_axis`). Series may target a secondary Y axis (`y_axis: "right"`). Required registry params return 422 when missing; stream-heavy charts are Redis-cached for 5 min.
