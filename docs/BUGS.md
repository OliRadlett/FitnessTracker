...
### BUG-088: Linkable-Activities Endpoint Returns 500 (MissingGreenlet on lazy `Activity.sources`)
- **Status:** FIXED
- **File:** `backend/app/api/lifting.py:181`, `backend/app/services/lifting.py:312`
- **Issue:** `GET /api/v1/lifting/sessions/{id}/linkable-activities` (used by the "Link Strava Activity" modal) did a bare `select(Activity)` then `ActivityRead.model_validate(a)`. `ActivityRead` requires the `sources` relationship, but in SQLAlchemy async a lazily-loaded relationship can't be accessed outside greenlet context — validation touched the un-loaded `Activity.sources` and raised `MissingGreenlet`, surfacing as a 500 ("Failed to load: API error: 500" in the modal). Every other endpoint that serializes `ActivityRead` eager-loads via `_enrich_activity_read()` (see `api/activities.py:186-198`); this endpoint was the lone exception.
- **Fix:** Added `selectinload(Activity.sources/route/lifting_session.sets)` to `find_linkable_activities` and switched `get_linkable_activities` to serialize via the shared `_enrich_activity_read` helper imported from `app.api.activities`.

### BUG-089: Live Lift Pending Deletes Re-Queued Forever (Undo Wedges Sync / Stuck Finish)
- **Status:** FIXED
- **Files:** `frontend/src/lib/lifting/useLiveSession.ts` (`mergeWithStorage`, `flush` Step 3), `frontend/src/lib/api/fetch.ts`
- **Issue:** `flush()` snapshots state, pushes sets/deletes, and after each success calls `commit()` → `mergeWithStorage(working)`, which merged `working.pendingDeletes` with the **not-yet-updated** `stored.pendingDeletes` via a set union. A delete that had just succeeded was still present in storage (storage is only rewritten by `commit`), so the union re-added it and `saveState` persisted it again. Every undo of a synced set therefore re-queued its remote id; the next flush re-issued `DELETE` for a row that no longer existed → `404` → `flush` threw at Step 3 **before** Step 4 (finish), setting `syncError` and retrying forever. Symptoms: the `⟳ N to sync` pill never cleared, `Sync failing — retry`, and Finish stuck on the finishing overlay (the finish PATCH was unreachable behind the failing delete). Reproduced with a hook-level test: `deleteLiftingSet` was called 4× (once per retry) instead of 1×.
- **Fix:** Track remote ids deleted by the current flush in a `processedDeletes` set and remove them from the merged queue (`mergeWithStorage(working, processedDeletes)`); completed deletes can never be re-queued from stale storage. Additionally, Step 3 now tolerates `404` (row already deleted from another tab/device) as success instead of retrying forever — `apiFetch` now attaches `err.status` so callers can distinguish "already gone" without string-matching. Regression tests: `frontend/src/__tests__/live-session-sync.test.tsx`.

### BUG-090: Live Lift Stale Flush Resurrects an Old Session Over a New One
- **Status:** FIXED
...
### BUG-098: Live Lift Wedged on "Finishing…"/"Loading…" So START Was Unreachable (mobile)
- **Status:** FIXED
- **Files:** `frontend/src/lib/lifting/useLiveSession.ts`, `frontend/src/components/lifting/LiveWorkout.tsx`, `frontend/src/app/(app)/lifting/live/page.tsx`
- **Issue:** A persisted `finishing` state could never resolve, leaving the full-screen finishing overlay up so a new session couldn't be started. Three causes: (1) `finishing` states saved before the `finish_requested` flag existed are skipped by both the 4s retry effect and `flush()` Step 4 (both gate on the flag) — a permanent dead-end; (2) if the remote session was deleted/auto-closed (`cleanup_old_data`), the finish `PATCH` 404s and retries forever; (3) on phones the `LiveWorkout` container had no `z-index`, so the app's `MobileBottomNav` (`z-40`) painted over the LOG SET button.
- **Fix:** `loadState()` backfills `finish_requested`/`endedAt` on legacy `finishing` states; `flush()` Step 4 runs for `finish_requested || phase === 'finishing'`, guards a null `sessionId`, and treats `PATCH` 404 as done; the retry effect also triggers on `phase === 'finishing'`; `startSession` adopts an active session written by another tab instead of silently no-op'ing; the live workout / finishing overlay / finish sheet are `z-50`. Regression tests in `frontend/src/__tests__/live-session-sync.test.tsx` (3 new).
