...
| `/lifting/videos` | `lifting/videos/page.tsx` | **§1.1 Video Bank** — list all strength videos with source/exercise/date filters + **quick-jump exercise chips**, preview modal, add-video modal (R2 upload only — URL mode removed 2026-09-09), per-video delete, AI form-analysis panel. **Polls every 5s while any video is queued/processing** so the status badge updates itself |
...
| `VideoEmbed` | **§1.1/§3.18** — R2 `<video>` player; presigned GET via `getVideoStreamUrl` with an **Original / Trimmed / Pose / Live pose** variant toggle (defaults to Original; Pose = server-rendered overlay MP4; **Live pose** = client-side `PoseCanvas` on the clean video, offered when `pose_track_r2_key` exists), retries failed loads |
| `PoseCanvas` | **§3.18 / F2** — Client-rendered skeleton + fading bar-path trail on a canvas over the `<video>`, synced to the playhead via rAF; reads the persisted pose track (T5). Letterbox-aware; skips low-visibility joints |
| `Pose3D` | **§3.18 / F2** — three.js 3D skeleton from the persisted metric world landmarks, synced to the playhead (OrbitControls, ground grid, hips lifted so feet sit on the grid). Shown as a PiP panel under the "3D" toggle; lazily imported `next/dynamic ssr:false` so `three` stays out of the first-load bundle |
| `PoseTimeline` | **§3.18 / F2** — per-rep chapters + a velocity bar per rep (bar height = concentric velocity), synced to the playhead; clicking a bar seeks the video. Overlaid at the bottom of the Live/3D view |
| `VelocitySparkline` | **§3.18 / F2** — per-frame bar-speed sparkline (derived from the persisted bar points) with a playhead marker, synced via rAF. Shown above the rep timeline in the Live/3D overlay |
...
| `LiftVideoForm` | **§1.1** — Add-strength-video modal: R2 presigned PUT with progress bar; exercise autocomplete, optional session/PR linkage, **set linkage** (picking a set of the chosen session autofills exercise/load/reps → `lifting_set_id`), expected-reps and **camera-angle** selectors (side unlocks sagittal form checks). Validation/upload errors render in an inline `role="alert"` banner (no native `alert()`) |
| `VideoEditModal` | **§1.1/§3.18** — Edit a video after upload (exercise, load, **expected reps** for detection vs **reps counted** override, camera angle, notes) via `PATCH /videos/{id}`; "Reprocess" re-runs analysis with the corrections |
