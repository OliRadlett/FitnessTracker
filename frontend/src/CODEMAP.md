...
| `RouteDetailPanel` | Slide-over detail panel with tabs (Overview, Map & Profile, History, Merged View, Weather, Effort), edit/favorite/delete actions. Map & Profile tab has a 2D/3D toggle — 3D lazy-loads `Route3D` |
| `MobileRouteDetailSheet` | Mobile (<lg) bottom-sheet wrapper around `RouteDetailPanel` — slides up from bottom with backdrop, swipe-down to dismiss, touch-draggable |
