...
export function ActivityAiAnalysisCard({ activityId }: ActivityAiAnalysisCardProps) {
  ...
  const mutation = useMutation({
    mutationFn: () => authFetch<LlmAnalysis>(`/api/v1/activities/${activityId}/ai-analysis`, { method: 'POST' }),
    onSuccess: (data) => {
      queryClient.setQueryData(queryKey, data);
    },
    onError: (err: Error) => {
      console.error('[ActivityAiAnalysisCard] Analysis failed:', err);
    },
  });
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between w-full">
          <CardTitle>🤖 AI Ride Analysis</CardTitle>
          <button
            onClick={() => mutation.mutate()}
            disabled={isAnalyzing}
            aria-label={analysis ? 'Re-analyze ride' : 'Analyze ride with AI'}
      ...
      {/* Loading initial cached analysis */}
      {isLoading && !analysis && (
      ...
      {!isLoading && !analysis && !isAnalyzing && !mutation.isError && (
        <div className="text-center py-6">
          ...
          <p className="text-muted text-sm">No AI analysis yet</p>
          <p className="text-muted text-xs mt-1">
            Click "Analyze with AI" to get AI-powered insights on this ride
          </p>
        </div>
      )}
      ...
      {isAnalyzing && (
        <div className="flex items-center gap-3 py-4">
          ...
          <p className="text-sm text-muted">
            Analyzing ride data with Gemini... This may take 10-20 seconds.
          </p>
        </div>
      )}
