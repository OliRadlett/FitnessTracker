...
from sqlalchemy import String, DateTime, Date, Float, ForeignKey, UniqueConstraint, func
...
class DailyMetric(Base):
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "metric_date", "source", name="uq_daily_metric_user_date_source"),
    )

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    metric_date: Mapped[date] = mapped_column(Date, nullable=False, index=True)
    source: Mapped[str] = mapped_column(String(50), nullable=False)  # whoop, strava, wahoo
