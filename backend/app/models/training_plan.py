...
class TrainingPlan(Base):
    ...
    plan_type: Mapped[str] = mapped_column(String(50), nullable=False, default="custom")  # custom, build, base, peak, taper, recovery
    ...
    days: Mapped[list["TrainingPlanDay"]] = relationship(back_populates="plan", cascade="all, delete-orphan", order_by="TrainingPlanDay.day_date")
...
class TrainingPlanDay(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    plan_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("training_plans.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    planned_type: Mapped[str] = mapped_column(String(20), nullable=False, default="rest")  # rest, easy, moderate, hard, race
