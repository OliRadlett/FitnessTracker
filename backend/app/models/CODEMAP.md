...
| `activity.py` | `Activity`, `ActivitySource`, `ActivityStream` | Activity has many Sources/Streams; optionally links to LiftingSession + Route. `Activity.tss_source` (QW4, migration 062: 'power'|'hr'|'provider'|'manual', NULL=unknown) records how `tss` was produced so the UI can badge power-TSS vs hrTSS |
