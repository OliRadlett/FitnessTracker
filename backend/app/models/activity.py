...
from sqlalchemy import String, DateTime, Float, Integer, ForeignKey, UniqueConstraint, func
...
class Activity(Base):
    ...
    __table_args__ = (
        UniqueConstraint("source", "provider_activity_id", name="uq_activity_source_provider_id"),
    )

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    connection_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("oauth_connections.id", ondelete="SET NULL"), nullable=True)
    route_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("routes.id", ondelete="SET NULL"), nullable=True, index=True)
    source: Mapped[str] = mapped_column(String(50), nullable=False)  # strava, wahoo, manual (primary source)
    provider_activity_id: Mapped[str | None] = mapped_column(String(255), nullable=True)
    sport_type: Mapped[str] = mapped_column(String(50), nullable=False, index=True)  # cycling, running, swimming, strength, powerlifting
    ...
    tss: Mapped[float | None] = mapped_column(Float, nullable=True)  # Training Stress Score
    ...
    lifting_session: Mapped["LiftingSession | None"] = relationship(back_populates="linked_activity")  # type: ignore[name-defined]
...
class ActivitySource(Base):
    ...
    provider: Mapped[str] = mapped_column(String(50), nullable=False)  # strava, wahoo, komoot
    provider_activity_id: Mapped[str] = mapped_column(String(255), nullable=False)
    provider_name: Mapped[str | None] = mapped_column(String(500), nullable=True)  # original name from provider
    raw_data: Mapped[dict | None] = mapped_column(JSONB, nullable=True)  # full API response
    synced_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class ActivityStream(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    activity_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("activities.id", ondelete="CASCADE"), nullable=False, index=True)
    stream_type: Mapped[str] = mapped_column(String(50), nullable=False)  # heartrate, power, cadence, altitude, velocity
    data: Mapped[dict] = mapped_column(JSONB, nullable=False)  # array of time-series values
    resolution: Mapped[int | None] = mapped_column(Integer, nullable=True)  # seconds per point
