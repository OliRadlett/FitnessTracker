...
class SleepLog(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "sleep_date", "source", name="uq_sleep_log_user_date_source"
        ),
    )
    ...
    @property
    def effective_total_sleep_seconds(self) -> int | None:
        """Return total_sleep_seconds, falling back to computation from bounds."""
        if self.total_sleep_seconds:
            return self.total_sleep_seconds
        if self.sleep_start and self.sleep_end:
            delta = int((self.sleep_end - self.sleep_start).total_seconds())
            awake = self.awake_seconds or 0
            if delta > awake:
                return delta - awake
        return None
