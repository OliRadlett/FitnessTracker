...
class Event(Base):
    ...
    event_type: Mapped[str] = mapped_column(
        String(50), nullable=False, default="race"
    )  # race, ride, lift, other
    target_tss: Mapped[float | None] = mapped_column(Float, nullable=True)
    taper_days: Mapped[int] = mapped_column(
        default=14
    )  # number of taper days before event
    notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    result: Mapped[dict | None] = mapped_column(
        JSONB, nullable=True
    )  # {finishing_time, finishing_position, class_position, personal_best, notes}
