...
MATCH_THRESHOLD = 0.60  # Minimum weighted score to consider a duplicate
...
async def find_duplicate_route(
    ...
    if best_score >= MATCH_THRESHOLD and best_route is not None:
...
async def create_or_merge_route(
    ...
    if duplicate:
        ...
        route = await create_route(
            ...
            elevation_profile, country, locality, raw_data,
        )
        # Add the source
        await add_route_source(
            db, route.id,
