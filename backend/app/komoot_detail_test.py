...
def main():
    with httpx.Client(timeout=15, follow_redirects=True) as client:
            ...
            for key in [
                ...
                if key in detail:
                    ...
                    if isinstance(val, str):
                        print(
                            f"  {key}: string of length {len(val)}, preview: {val[:100]}..."
                        )
                ...
                for ek, ev in embedded.items():
                    if isinstance(ev, dict):
                        ...
                        if "coordinates" in ev:
                            ...
                            print(
                                f"    coordinates: {len(coords)} items, first: {coords[0] if coords else 'empty'}"
                            )
        ...
        resp = client.get(
            f"{BASE}/tours/{tour_id}/coordinates/", headers=auth_headers()
        )
        print(f"Status: {resp.status_code}")
        if resp.status_code == 200:
            coords = resp.json()
